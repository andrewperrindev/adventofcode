...
async function main() {
  ...
  // PART 2: The only change needed to get the result for part 2 is to reverse the list.
  // This ends up extrapolating an earlier reading, instead of a later reeading.
  const oasisInputNums = oasisInput.map((input) => input.split(/\s+/).map((num) => parseInt(num, 10)).reverse());
