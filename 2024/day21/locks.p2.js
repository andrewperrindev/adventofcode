...
const convertDirectionsToCommands = (directionChars, initChar = BTN_A) => {
    let allCommandPaths = [];

    for (let charDirPath of directionChars) {
        let commands = [];
        let prevChar = initChar;
...
const findSegment = (commandString, start = 0) => {
    const index = commandString.indexOf(BTN_A, start);
    return [start, index];
};

const calculateCommandsNTimes = (commands, target = 25) => {
    const cache = {};

    let tally = 0;
    let [start, idx] = findSegment(commands);

    // Each segment ends with an 'A' command.
    // Continue until there are no segments left.
    while (idx >= 0) {
        const newSegment = commands.slice(start, idx + 1);
        const length = calculateCommandsRecursive(newSegment, cache, target - 1);

        [start, idx] = findSegment(commands, idx + 1);
        tally += length;
    }
...
const calculateCommandsRecursive = (segment, cache, target, depth = 1) => {
    // Without a cache, this method still takes too long. Segments repeat often, so we can
    // save a lot of work by caching their final length (starting from a particular depth).
    const cacheKey = `${segment.join(',')}.${depth}`;

    if (cache[cacheKey] !== undefined) {
        return cache[cacheKey];
    }

    // Find all possible ways to input this segment as a series of commands.
    const paths = convertDirectionsToCommands([segment]);

    if (depth === target) {
        // If we've reached the target depth, find the smallest number of
        // commands, cache the result, and return.
        const result = paths.reduce((lowest, path) => {
        ...
        // Otherwise, for each path, break it into individual segments and repeat.
        const results = paths.map((path) => {
            let tally = 0;
            let [start, idx] = findSegment(path);

            while (idx >= 0) {
                const newSegment = path.slice(start, idx + 1);
                const length = calculateCommandsRecursive(newSegment, cache, target, depth + 1);

                [start, idx] = findSegment(path, idx + 1);
                tally += length;
            }
        ...
        // Cache & return the smallest number of commands found for this segment's paths.
        const result = results.reduce((lowest, tally) => {
            return tally < lowest ? tally : lowest;
        }, Infinity);
...
const getResult = async (type = 'example') => {
    ...
    for (let code of input) {
        ...
        // Trying to calculate the command expansions 25 times for the whole command string
        // (as in part 1) is too slow. Instead, break down the entire string into segments &
        // process each segment independently (since they have no effect on each other).
        const results = arrowCommands.map((arrowCommand) => calculateCommandsNTimes(arrowCommand));

        const lowest = results.reduce((lowest, score) => {
            return score < lowest ? score : lowest;
        }, Infinity);
