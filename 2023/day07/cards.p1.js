...
async function main() {
  ...
  try {
    ...
    handsWithBids = data.split(EOL).reduce((prev, current) => {
      prev.push(current);
      return prev;
    }, []);
  }
  ...
  const hands = parseCardHands(handsWithBids);
  ...
  console.log(`TOTAL: ${rankedHands.reduce((prev, current, index) => prev + (current.bid * (index + 1)), 0)}`);
}
